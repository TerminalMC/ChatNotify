...
public class TextField extends EditBox {
    ...
    public void moveCursorTo(int delta, boolean select) {
        ...
        onValueChange(((EditBoxAccessor) this).chatnotify$getValue());
    }

    public void moveCursorToEnd(boolean select) {
        this.moveCursorTo(((EditBoxAccessor) this).chatnotify$getValue().length(), select);
    }

    private void onValueChange(String newText) {
        Consumer<String> responder = ((EditBoxAccessor) this).chatnotify$getResponder();
        if (responder != null) {
